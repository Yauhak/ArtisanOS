...
/* 全部重启：重置整个堆，再按 SCHEDULE 重新装载 */
ars_i8 arssched_rebootAll(void) {
	init_mem_info();
	return arssched_load();
}

/* 活着的判定：槽位里的 CurCmd 非空，说明这个任务还在往下取指令 */
...
static int freeSlot(void) {
	for (int t = 0; t < OS_MAX_TASK; t++)
		if (!CurCmd[t]) return t;
	return -1;
}

int arssched_alive(int id) {
	return (id >= 0 && id < OS_MAX_TASK && CurCmd[id]) ? 1 : 0;
}

const char *arssched_name(int id) {
	static const char empty[] = "";
	if (id < 0 || id >= OS_MAX_TASK) return empty;
	return taskName[id];
}

int arssched_count(void) { return aliveCount(); }
...
ars_i8 arssched_start(const char *name) {
	...
	long got = readFile(name, prog, FILE_MAX);
	if (got <= 0) return FS_ENOENT;              /* 文件不存在或读不出来 */
	if (got > OS_MAX_SGL_PG) return FS_ECORRUPT; /* 超出任务代码页 */
	...
	needJump[t] = 0;
	int j = 0;
	for (; j < NAME_LEN && name[j]; j++) taskName[t][j] = name[j];
	taskName[t][j] = 0;
	gSchedTasks = (uars_i8)aliveCount();
	return FS_OK;
}

ars_i8 arssched_killId(uars_i8 id) {
	if (id >= OS_MAX_TASK) return FS_ENOENT;
	if (!CurCmd[id] && !taskName[id][0]) return FS_ENOENT; /* 空槽位 */
	ReArrangeMemAndTask(id); /* 释放该任务占的块并清零槽位，不碰别的任务 */
	CurCmd[id] = 0;
	needJump[id] = 0;
	taskName[id][0] = 0;
	gSchedTasks = (uars_i8)aliveCount();
...
ars_i8 arssched_kill(const char *name) {
	...
	int killed = 0;
	for (int t = 0; t < OS_MAX_TASK; t++) {
		if (!taskName[t][0]) continue;              /* 空槽位 */
		if (!nameSame(taskName[t], name)) continue; /* 不区分大小写的全等 */
		if (arssched_killId((uars_i8)t) == FS_OK) killed++;
	}
	if (!killed) return FS_ENOENT;
	gSchedTasks = (uars_i8)aliveCount();
	return FS_OK;
}
...
int arssched_count(void) { return 0; }
#endif
