 ...
 * 文件名比较**不区分大小写**（终端上大小写混着敲都能命中），存的时候按原样存。
 *
 ...
 *
 * 页分配就是最朴素的首次适配：从 0 号页往后扫，遇到第一个空闲页就用掉。
 * 文件页不要求连续（没有外部碎片），加链也只是改 RAM 里的 nextPg。
 *
 * 底层必须先擦后写，而擦除粒度是 4KB 扇区（比页大），所以 arsfs_flash_write
 * 统一走"读回整扇区 -> 覆盖对应区间 -> 擦除扇区 -> 整扇区写回"。
 * 注意是【覆盖】而不是按位与：目标是从 FLASH 读回的旧内容，
 * 按位与会把上一版数据残留在页里。
 *
 * 代价：同一个扇区里可能同时住着好几个文件的页，而"擦除 + 写回"不是原子的。
 * 如果正好在这个窗口里掉电/复位，同扇区的其它文件也会被清掉。这是为了简单
 * 直接接受的风险，不为此增加独占扇区之类的额外机制。
 * ================================================================ */
