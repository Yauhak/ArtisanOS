...
> **自己用命令行烧 UF2 的话注意：`elf2uf2` 不会覆盖已存在的输出文件，而且失败时
> 一声不吭。** 必须先删掉旧的 `main.ino.uf2` 再生成，否则你会拿着上一次的镜像
> 反复烧，然后对着板子怀疑人生。（`ars_tools/arm_link.ps1` 已经处理了这一点，
> 它编完会自己出 UF2 并打印 `UF2 OK: ...`。）
...
- **一次最多推 63 字节**（`TX_PKT`）。这是踩出来的坑：写入长度正好等于端点最大包
  ...
  而且只在应答总长恰好是 64 的倍数时出现。留 1 字节让每包都是短包即可；
...
> （`LEDStream` 命中 `LEDSTREAM`）都**不区分大小写**，`restart` 是 `reboot` 的同义词。
